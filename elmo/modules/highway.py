...
class HighWay(nn.Cell):
    ...
    def __init__(self, input_dim: int, num_layers: int=1, activation:str='relu'):
        ...
        for _ in range(num_layers):
            carry = nn.Dense(input_dim, input_dim, weight_init=Normal(np.sqrt(1.0 / input_dim)), bias_init=Constant(-2.0)).to_float(mindspore.float16)
            transform = nn.Dense(input_dim, input_dim, weight_init=Normal(np.sqrt(1.0 / input_dim))).to_float(mindspore.float16)
