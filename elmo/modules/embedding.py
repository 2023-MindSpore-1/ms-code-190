...
class CharacterEncoder(nn.Cell):
    ...
    def __init__(self, 
        ...
        # highway layers
        self._highways = HighWay(n_filters, n_highway, 'relu').to_float(mindspore.float16)
        # projection layer
        self._projection = nn.Dense(n_filters, output_dim, has_bias=True, weight_init=Normal(np.sqrt(1.0 / n_filters))).to_float(mindspore.float16)
        # array operations
