...
class LanguageModel(nn.Cell):
    def __init__(self, options, training):
        ...
        self.char_embedding = CharacterEncoder(filters, n_filters, max_chars, char_embed_dim, n_chars, n_highway, projection_dim, activation).to_float(mindspore.float16)
        self.bilstm = ELMoLSTM(projection_dim, lstm_dim, projection_dim, n_lstm_layers, keep_prob, cell_clip, proj_clip, use_skip_connections, is_training=True, batch_first=True).to_float(mindspore.float16)
    ...
    def construct(self, inputs, inputs_backward, next_ids_forward, next_ids_backward):
        ...
        """
        # sample_values = (s0, s1, s2)
        # sample_values_reverse = (sv0, sv1, sv2)
        ...
        token_embedding = self.cast(token_embedding, mindspore.float32)
        # token_embedding_backward = self.cast(token_embedding_backward, mindspore.float32)
